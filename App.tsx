...
import type { CampaignAnalysisResult, CTA, MarketingObjective, CampaignGoalDetails } from './types';
  ...
  const handleAnalysis = useCallback(async (
    ...
    try {
      if (campaigns.length === 0) {
        throw new Error("Please provide at least one campaign message.");
      }
      if (keywords.length === 0) {
        throw new Error("Please provide keywords for trend analysis.");
      }
      if (ctas.length === 0) {
        throw new Error("Please provide CTAs for shareability analysis.");
      }
      
      const results = await analyzeCampaignsWithGemini(campaigns, keywords, ctas, objective, details);
